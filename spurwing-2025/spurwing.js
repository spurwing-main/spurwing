function main() {
	...
	function game() {
		...
		let engine, render, resizeObserver, projectile, elastic;
		...
		const createProjectile = (spriteUrl, CONFIG) => {
			const body = Bodies.circle(
				CONFIG.slingshot.anchor.x,
				CONFIG.slingshot.anchor.y,
				CONFIG.slingshot.radius,
				{
					...
					render: {
						sprite: {
							texture: spriteUrl,
							xScale: (CONFIG.slingshot.radius * 2) / 112,
							yScale: (CONFIG.slingshot.radius * 2) / 112,
						},
					},
				}
		...
		const addSlingshot = (CONFIG) => {
			const spriteUrl = randomFrom(SVG_PROJECTILES);
			projectile = createProjectile(spriteUrl, CONFIG);
			elastic = Constraint.create({
				pointA: CONFIG.slingshot.anchor,
				bodyB: projectile,
				length: CONFIG.slingshot.constraintLength,
		...
		const initSimulation = async (width, height, CONFIG) => {
			...
			const ground = Bodies.rectangle(
				...
				width,
				CONFIG.ground.thickness,
				{ isStatic: true, render: { fillStyle: CONFIG.ground.fillStyle } }
			);

			const ceiling = Bodies.rectangle(
				width / 2,
				-CONFIG.ceiling.yOffset - CONFIG.ceiling.thickness / 2,
				width,
				CONFIG.ceiling.thickness,
				{ isStatic: true, render: { fillStyle: CONFIG.ceiling.fillStyle } }
			);

			const leftWall = Bodies.rectangle(
				-CONFIG.walls.thickness / 2 + CONFIG.walls.visibleThickness,
				height / 2,
				CONFIG.walls.thickness,
				height + CONFIG.ceiling.yOffset,
				{ isStatic: true, render: { fillStyle: CONFIG.walls.fillStyle } }
			);
			...
			const targetPlatform = Bodies.rectangle(
				...
				CONFIG.platform.width,
				CONFIG.platform.height,
				{ isStatic: true, render: { fillStyle: CONFIG.platform.fill } }
			);

			const blockCols = Math.floor(
				(CONFIG.platform.width + CONFIG.blocks.spacing) /
					(CONFIG.blocks.width + CONFIG.blocks.spacing)
			);
			const startX =
				CONFIG.platform.x -
				(blockCols / 2) * (CONFIG.blocks.width + CONFIG.blocks.spacing) +
				(CONFIG.blocks.width + CONFIG.blocks.spacing) / 2;
			const startY = CONFIG.platform.y - CONFIG.platform.height / 2 - CONFIG.blocks.height / 2;

			const blocks = Composite.create();
			for (let i = 0; i < CONFIG.blocks.rows; i++) {
				for (let j = 0; j < blockCols; j++) {
					const x = startX + j * (CONFIG.blocks.width + CONFIG.blocks.spacing);
					const y = startY - i * (CONFIG.blocks.height + CONFIG.blocks.spacing);
					const block = createBlock(x, y, randomFrom(SVG_BLOCKS), CONFIG.blocks.width);
			...
			Events.on(engine, "afterUpdate", () => {
				if (
					...
					Body.setAngularVelocity(projectile, (Math.random() - 0.5) * 0.4);

					projectile = createProjectile(randomFrom(SVG_PROJECTILES), CONFIG);
		...
		const resizeAwareInit = () => {
			...
			initSimulation(width, height, CONFIG);
		};
